#ifndef LATTIS_RUST_XML_GRAPH_HPP
#define LATTIS_RUST_XML_GRAPH_HPP
...
inline bool read_xml(const std::string& xml, const std::string& name, graph& lat) {
  auto raw = lattis_graph_from_xml(xml.c_str(), name.c_str());
  if (!raw) {
    return false;
  }
  lat = graph(raw->dim);
...
inline std::string write_xml(const std::string& name, const graph& lat) {
  ...
  return detail::c_string(lattis_graph_to_xml(name.c_str(), &raw));
}
