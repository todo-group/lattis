#ifndef LATTIS_RUST_XML_UNITCELL_HPP
#define LATTIS_RUST_XML_UNITCELL_HPP
...
inline bool read_xml(const std::string& xml, const std::string& name, unitcell& cell) {
  auto raw = lattis_unitcell_from_xml(xml.c_str(), name.c_str());
  if (!raw) {
    return false;
  }
...
inline std::string write_xml(const std::string& name, const unitcell& cell) {
  ...
  return detail::c_string(lattis_unitcell_to_xml(name.c_str(), &raw));
}
