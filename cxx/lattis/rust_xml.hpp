#ifndef LATTIS_RUST_XML_HPP
#define LATTIS_RUST_XML_HPP
...
inline std::string last_error_message() {
  return c_string(lattis_last_error_message());
}

} // namespace detail

inline bool read_xml(const std::string& xml, const std::string& name, basis& bs) {
  auto raw = lattis_basis_from_xml(xml.c_str(), name.c_str());
  if (!raw) {
    return false;
  }
...
inline std::string write_xml(const std::string& name, const basis& bs) {
  ...
  return detail::c_string(lattis_basis_to_xml(name.c_str(), &raw));
}

} // end namespace lattis
