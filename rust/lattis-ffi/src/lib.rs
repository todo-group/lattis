use lattis::{
    read_basis_from_str, read_graph_from_str, read_unitcell_from_str, write_basis_to_string,
    write_graph_to_string, write_unitcell_to_string, Basis, BasisMatrix, CoordinateVector, Graph,
    OffsetVector, Unitcell,
};
...
#[repr(C)]
pub struct lattis_unitcell_raw {
    pub dim: usize,
    pub num_sites: usize,
...
#[no_mangle]
pub extern "C" fn lattis_string_free(ptr: *mut c_char) {
    ffi_void(|| {
        if !ptr.is_null() {
            unsafe {
...
#[no_mangle]
pub extern "C" fn lattis_last_error_message() -> *mut c_char {
    ffi_ptr("lattis_last_error_message", || {
...
#[no_mangle]
pub extern "C" fn lattis_basis_raw_free(ptr: *mut lattis_basis_raw) {
    ffi_void(|| {
        if ptr.is_null() {
            return;
        }
...
#[no_mangle]
pub extern "C" fn lattis_unitcell_raw_free(ptr: *mut lattis_unitcell_raw) {
    ffi_void(|| {
        if ptr.is_null() {
            return;
        }
        unsafe {
            ...
            if !raw.site_coordinates.is_null() {
                drop(Vec::from_raw_parts(
                    raw.site_coordinates,
                    raw.site_coordinates_len,
                    raw.site_coordinates_len,
                ));
            }
...
#[no_mangle]
pub extern "C" fn lattis_graph_raw_free(ptr: *mut lattis_graph_raw) {
    ffi_void(|| {
        if ptr.is_null() {
            return;
        }
...
#[no_mangle]
pub extern "C" fn lattis_basis_from_xml(
    xml: *const c_char,
    name: *const c_char,
) -> *mut lattis_basis_raw {
    ffi_ptr("lattis_basis_from_xml", || {
...
#[no_mangle]
pub extern "C" fn lattis_unitcell_from_xml(
    xml: *const c_char,
    name: *const c_char,
) -> *mut lattis_unitcell_raw {
    ffi_ptr("lattis_unitcell_from_xml", || {
        with_strs(xml, name, |xml, name| {
...
#[no_mangle]
pub extern "C" fn lattis_graph_from_xml(
    xml: *const c_char,
    name: *const c_char,
) -> *mut lattis_graph_raw {
    ffi_ptr("lattis_graph_from_xml", || {
        with_strs(xml, name, |xml, name| {
...
#[no_mangle]
pub extern "C" fn lattis_basis_to_xml(
    name: *const c_char,
    raw: *const lattis_basis_raw,
) -> *mut c_char {
    ffi_ptr("lattis_basis_to_xml", || unsafe {
        if name.is_null() {
            return ptr::null_mut();
        }
...
#[no_mangle]
pub extern "C" fn lattis_unitcell_to_xml(
    name: *const c_char,
    raw: *const lattis_unitcell_raw,
) -> *mut c_char {
    ffi_ptr("lattis_unitcell_to_xml", || unsafe {
        if name.is_null() {
            return ptr::null_mut();
        }
        let name = match CStr::from_ptr(name).to_str() {
...
#[no_mangle]
pub extern "C" fn lattis_graph_to_xml(
    name: *const c_char,
    raw: *const lattis_graph_raw,
) -> *mut c_char {
    ffi_ptr("lattis_graph_to_xml", || unsafe {
        if name.is_null() {
            return ptr::null_mut();
        }
        let name = match CStr::from_ptr(name).to_str() {
...
fn with_strs<T>(
    xml: *const c_char,
    name: *const c_char,
    f: impl FnOnce(&str, &str) -> Option<T>,
) -> *mut T {
    unsafe {
...
fn box_basis_raw(basis: &Basis) -> lattis_basis_raw {
...
fn basis_from_raw(raw: &lattis_basis_raw) -> Option<Basis> {
...
fn box_unitcell_raw(cell: &Unitcell) -> lattis_unitcell_raw {
...
fn unitcell_from_raw(raw: &lattis_unitcell_raw) -> Option<Unitcell> {
    if raw.site_types.is_null()
        || raw.site_coordinates.is_null()
        || raw.bond_sources.is_null()
        || raw.bond_targets.is_null()
        || raw.bond_types.is_null()
        || raw.bond_offsets.is_null()
    {
        return None;
    }
    if raw.site_coordinates_len != raw.num_sites * raw.dim
        || raw.bond_offsets_len != raw.num_bonds * raw.dim
    {
        return None;
    }
    unsafe {
        let site_types = std::slice::from_raw_parts(raw.site_types, raw.num_sites);
        let site_coordinates =
            std::slice::from_raw_parts(raw.site_coordinates, raw.site_coordinates_len);
...
fn box_graph_raw(graph: &Graph) -> lattis_graph_raw {
...
fn graph_from_raw(raw: &lattis_graph_raw) -> Option<Graph> {
    if raw.site_types.is_null()
        || raw.site_coordinates.is_null()
        || raw.bond_sources.is_null()
        || raw.bond_targets.is_null()
