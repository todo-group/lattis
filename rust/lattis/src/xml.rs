...
pub fn read_basis_from_str(xml: &str, name: &str) -> Result<Basis> {
    ...
    let entry = find_entry(&doc, "LATTICE", name)?
        .ok_or_else(|| XmlError::UnknownEntry(name.to_string()))?;
    parse_basis(entry)
}

pub fn read_unitcell_from_str(xml: &str, name: &str) -> Result<Unitcell> {
    let doc = Document::parse(xml)?;
    let entry = find_entry(&doc, "UNITCELL", name)?
        .ok_or_else(|| XmlError::UnknownEntry(name.to_string()))?;
...
pub fn read_graph_from_str(xml: &str, name: &str) -> Result<Graph> {
    let doc = Document::parse(xml)?;
    let entry =
        find_entry(&doc, "GRAPH", name)?.ok_or_else(|| XmlError::UnknownEntry(name.to_string()))?;
    parse_graph(entry)
}
...
pub fn write_unitcell_to_string(name: &str, cell: &Unitcell) -> String {
    ...
    let _ = writeln!(
        xml,
        "  <UNITCELL name=\"{}\" dimension=\"{}\" vertices=\"{}\">",
        escape_attr(name),
        cell.dimension(),
        cell.num_sites()
    );
...
fn parse_basis(entry: Node<'_, '_>) -> Result<Basis> {
    ...
    let basis_node = entry
        .children()
        .find(|child| child.is_element() && child.tag_name().name() == "BASIS")
        .ok_or(XmlError::InvalidFormat("missing BASIS element"))?;
    let mut vectors = Vec::new();
    for vector_node in basis_node
        .children()
        .filter(|child| child.is_element() && child.tag_name().name() == "VECTOR")
    {
...
fn parse_unitcell(entry: Node<'_, '_>) -> Result<Unitcell> {
        ...
        .filter(|child| child.is_element() && child.tag_name().name() == "VERTEX")
    {
        ...
        for coordinate_node in vertex
            .children()
            .filter(|child| child.is_element() && child.tag_name().name() == "COORDINATE")
        {
            if found_coordinate {
            ...
            if values.len() != dimension {
                return Err(XmlError::DimensionMismatch(
                    "site coordinate dimension mismatch",
                ));
            }
    ...
    for edge in entry
        .children()
        .filter(|child| child.is_element() && child.tag_name().name() == "EDGE")
    {
        ...
        for source_node in edge
            .children()
            .filter(|child| child.is_element() && child.tag_name().name() == "SOURCE")
        {
            if let Some(vertex) = source_node.attribute("vertex") {
                let parsed = vertex
                    .parse::<usize>()
                    .map_err(|_| XmlError::InvalidFormat("invalid SOURCE vertex"))?;
                if parsed == 0 {
            ...
            if let Some(offset) = source_node.attribute("offset") {
                ...
                if values.len() != dimension {
                    return Err(XmlError::DimensionMismatch(
                        "SOURCE offset dimension mismatch",
                    ));
                }
                for (index, value) in values.into_iter().enumerate() {
        ...
        for target_node in edge
            .children()
            .filter(|child| child.is_element() && child.tag_name().name() == "TARGET")
        {
            if let Some(vertex) = target_node.attribute("vertex") {
                let parsed = vertex
                    .parse::<usize>()
                    .map_err(|_| XmlError::InvalidFormat("invalid TARGET vertex"))?;
                if parsed == 0 {
            ...
            if let Some(offset) = target_node.attribute("offset") {
                ...
                if values.len() != dimension {
                    return Err(XmlError::DimensionMismatch(
                        "TARGET offset dimension mismatch",
                    ));
                }
                for (index, value) in values.into_iter().enumerate() {
...
fn parse_graph(entry: Node<'_, '_>) -> Result<Graph> {
        ...
        .filter(|child| child.is_element() && child.tag_name().name() == "VERTEX")
    {
        ...
        let coordinate = if dimension > 0 {
            let coordinate_node = vertex
                .children()
                .find(|child| child.is_element() && child.tag_name().name() == "COORDINATE")
...
fn parse_edge_vertex(edge: Node<'_, '_>, tag: &'static str) -> Result<usize> {
    ...
    let vertex = node
        .attribute("vertex")
        .ok_or(XmlError::MissingAttribute("vertex"))?
        .parse::<usize>()
        .map_err(|_| XmlError::InvalidFormat("invalid vertex"))?;
    if vertex == 0 {
    ...
    T: std::str::FromStr,
{
    ...
    for token in text.split_whitespace() {
        values.push(
            token
                .parse::<T>()
                .map_err(|_| XmlError::InvalidFormat("invalid numeric value"))?,
        );
    }
    Ok(values)
}
