...
impl Graph {
    ...
    pub fn from_basis_unitcell_extent(
        ...
        let num_cells = extent
            .iter()
            .fold(1usize, |acc, value| acc * (*value as usize));

        for cell_index in 0..num_cells {
            let cell_offset = index_to_offset(cell_index, extent);
            let cell_offset_f =
                CoordinateVector::from_iterator(dim, cell_offset.iter().map(|value| *value as f64));
            for site in 0..cell.num_sites() {
                let coordinate = basis.basis_vectors()
                    * (cell_offset_f.clone() + cell.site(site).coordinate.clone());
