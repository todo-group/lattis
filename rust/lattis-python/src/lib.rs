...
impl PyGraph {
    ...
    fn from_basis_unitcell_extent(
        ...
        validate_lattis_inputs(&basis.inner, &unitcell.inner, &extent, boundary.len())?;
...
#[pymodule]
fn lattis(m: &Bound<'_, PyModule>) -> PyResult<()> {
...
fn core_value<T>(f: impl FnOnce() -> PyResult<T>) -> PyResult<T> {
    match catch_unwind(AssertUnwindSafe(f)) {
        Ok(result) => result,
        Err(_) => Err(PyRuntimeError::new_err("lattis panicked")),
    }
}
